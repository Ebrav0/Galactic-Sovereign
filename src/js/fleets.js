...
export function playerShipsAtSystem(state, systemId) {
  return state.playerShips.filter((s) => s.systemId === systemId && !s.transit && s.hp > 0);
}
...
export function playerShipEtaMs(state, ship) {
  const speed = shipLaneSpeed(ship.hull);
  return transitEtaMs(ship.transit, state.galaxy, state.time, speed, SHIP_LANE_MIN_LEG_MS);
}
...
export function orderShipTravel(state, shipId, targetId) {
  ...
  ship.transit = {
    ...
    legDurationMs: legDurationMs(state.galaxy, path[0], path[1], speed, SHIP_LANE_MIN_LEG_MS),
  };
  ship.systemId = null;
...
export function playerShipTransitPositions(state) {
  ...
  for (const ship of state.playerShips) {
    if (!ship.transit) continue;
    const status = playerShipStatus(ship, state.galaxy, state.time);
