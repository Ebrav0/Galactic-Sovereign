...
/** Keep-out radius aligned with rendered star corona / black-hole glow. */
export function starKeepOutRadius(system) {
  ...
  if (star.kind === 'blackhole') {
    return baseR * 4.2 + STAR_KEEP_OUT_PAD;
  }
...
export function keepOutRepulsion(state, system, x, y, strength = KEEP_OUT_REPULSION) {
  ...
  const starR = starKeepOutRadius(system);
  ({ ax, ay } = addRepulsion(ax, ay, x, y, 0, 0, starR, strength));
