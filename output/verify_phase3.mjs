// Phase 3 verification: Dyson loop + Solarii dual currency.
...
// --- Section 1: Save v5 shape ---
let s = await text();
check('1.1 saveVersion is 5', s.saveVersion === 5);
...
await page.evaluate(() => {
  const st = window.getGameState();
  st.solarii = 12.5;
  st.systems[st.stronghold].dyson.completedShells = 2;
});
...
s = await text();
check('1.4 save-v5 round trip restores dyson + solarii', s.solarii === 12.5 && s.dyson.completedShells === 2);

// v4 -> v5 migration
await page.evaluate(() => window.__newGame(42));
const v4State = await page.evaluate(() => {
  const st = window.getGameState();
  st.playerShips = [{ id: 'mig-ship', hull: 'corvette', systemId: st.stronghold, hp: 100, maxHp: 120, transit: null }];
  delete st.solarii;
  delete st.solariiUnlocked;
  for (const sys of Object.values(st.systems)) delete sys.dyson;
  return JSON.parse(JSON.stringify(st));
});
const v4StateJson = JSON.stringify(v4State);
const v4Envelope = JSON.stringify({
  saveVersion: 4,
  checksum: crc32(v4StateJson),
  savedAt: Date.now(),
  state: v4State,
});
await page.evaluate((raw) => localStorage.setItem('gs-save-slot-2', raw), v4Envelope);
await page.evaluate(() => window.__loadSlot('slot-2'));
s = await text();
check('1.5 v4 migrates to v5', s.saveVersion === 5 && s.playerShips.some((sh) => sh.id === 'mig-ship'));
...
await page.evaluate(() => {
  const st = window.getGameState();
  st.systems[st.stronghold].dyson.completedShells = 1;
});
s = await text();
...
await page.evaluate(() => {
  window.getGameState().systems[window.getGameState().stronghold].dyson.completedShells = 2;
});
...
const neighborId = await page.evaluate(() => {
  const st = window.getGameState();
  return st.galaxy.stars.find((x) => x.id !== st.stronghold).id;
});
await page.evaluate(([nid]) => {
  ...
  st.playerShips = [
    { id: 'cap-d1', hull: 'destroyer', systemId: nid, hp: 350, maxHp: 350, transit: null },
    { id: 'cap-d2', hull: 'destroyer', systemId: nid, hp: 350, maxHp: 350, transit: null },
    { id: 'cap-d3', hull: 'destroyer', systemId: nid, hp: 350, maxHp: 350, transit: null },
    { id: 'cap-d4', hull: 'destroyer', systemId: nid, hp: 350, maxHp: 350, transit: null },
    { id: 'cap-f1', hull: 'frigate', systemId: nid, hp: 200, maxHp: 200, transit: null },
    { id: 'cap-f2', hull: 'frigate', systemId: nid, hp: 200, maxHp: 200, transit: null },
  ];
  st.flagship.systemId = nid;
  st.flagship.transit = null;
  st.pirates.fleets.forEach((f) => { f.systemId = 'core'; f.transit = null; });
}, [neighborId]);
...
const persisted = await page.evaluate(([nid]) => {
  const sys = window.getGameState().systems[nid];
